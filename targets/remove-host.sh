...
if [ "$CLEAN" = true ]; then
    ...
    # Delete series by instance label (now set to vm_name)
    if curl -sf -X POST "${PROMETHEUS_URL}/api/v1/admin/tsdb/delete_series" --data-urlencode "match[]={instance=\"${VM_NAME}\"}" 2>/dev/null; then
        print_success "Deleted series matching instance=\"${VM_NAME}\""
    else
        print_warning "Could not delete series — is Prometheus running?"
    fi

    # Also clean by vm_name label
    curl -sf -X POST "${PROMETHEUS_URL}/api/v1/admin/tsdb/delete_series" --data-urlencode "match[]={vm_name=\"${VM_NAME}\"}" 2>/dev/null || true
